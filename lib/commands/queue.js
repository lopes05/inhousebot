        ...
        if (enforceWordCount(userMessage, 2)) {
            ...
            for (const [index, playerObj] of queuedPlayers.entries()) {
                if (userID === playerObj.discordId) {
                    ...
                    return {
                        responseMessage: `${playerObj.username} has left the queue. Players in queue: ${
        ...
        if (enforceWordCount(userMessage, 2)) {
            ...
            try {
                ...
                if (queuedPlayers.length === 6) {
                    ...
                    return {
                        responseMessage:
                            `${player.username} has joined the queue. Players in queue: 6. Beginning Draft.`,
                        deleteSenderMessage: true
                    };
                }
                ...
                return {
                    responseMessage: `${player.username} has joined the queue. Players in queue: ${
                        queuedPlayers.length
                    }.`,
                    deleteSenderMessage: true
                };
